...
# ORIGINAL_LANGUAGE = "bn-IN"  # Input language for speech recognition
# TARGET_LANGUAGE = "en-US"    # Target language for translation and TTS
...
def listen_print_loop(responses):
    ...
    if "exit" in transcript.lower():
        MoM = generate_meeting_summary(meeting_transcript)
        send_mail(MoM)
        exit()
...
def generate_meeting_summary(transcript_list):
    ...
    prompt = f"""
        Below is a transcript of a meeting. Summarize the key points, decisions, and action items in a formal and structured manner.
        The output should be in HTML format with inline CSS styling for a professional email UI. Use headings, bullet points, and sections for better readability.
        Ensure the following structure:

        - Title: "Minutes of Meeting (MoM)"
        - Date and Time of the Meeting
        - Participants (as a list)
        - Agenda (as bullet points)
        - Key Points Discussed (as bullet points)
        - Decisions Made (as bullet points)
        - Action Items (as numbered points)
        - Footer indicating it was auto-generated.

        Use a visually appealing color scheme, rounded edges for the title, and proper indentation for the content.
